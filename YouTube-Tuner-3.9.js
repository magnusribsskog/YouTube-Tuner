// ==UserScript==
// @name         YouTube Tuner v3.9
// @version      3.9
// @description  Self-healing selector corrections — filters homepage and search video cards via PHRASE, SLOP, CAPS, DUPE, WATCHED heuristics
// @author       Anonymous
...
/*
 * YouTube Tuner v3.9 — Self-Healing Selector Corrections
 * =====================================================================
 *
 ...
 *   - Unknown tag with ≥2 beacon confirmations → persisted to localStorage
...
(function() {
    ...
    // never modified at runtime. localStorage corrections are merged on top of
    // this set to produce activeContainerTags. If localStorage is cleared,
    ...
    function loadSelectorCorrections() {
        try {
    ...
    function saveSelectorCorrections(corrections) {
        try {
    ...
    function commitCorrection(tag) {
        ...
        saveSelectorCorrections(corrections);
        buildActiveTagSet();
    ...
    function bootOnce() {
        ...
        injectUIIfNeeded();
        const corrections = loadSelectorCorrections();
        console.log(`[YT-PURGE] v3.9 booting. Context: ${getPageContext()}. Custom phrases: ${customPhrases.length}. Nuke log: ${loadNukeLog().length} entries. Corrections: ${corrections.length}`);
    ...
    function init() {
        // Build the active tag set — baseline merged with any stored corrections
        buildActiveTagSet();
